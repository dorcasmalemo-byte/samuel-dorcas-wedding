window.WEDDING_CONFIG = {
  weddingDate: "2026-12-26T15:00:00+02:00",
  links: {
    maps: "https://www.google.com/maps/search/?api=1&query=Avianto+Johannesburg+South+Africa",
    accommodation: "https://book.nightsbridge.com/29945"
  },
  rsvp: {
    endpoint: "https://formsubmit.co/ajax/dorcasmalemo@icloud.com"
  },
  images: {
    hero:
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/0375ce1642f119ca4a478df665223d20/revisions/bd8ba3b7b25aae170b6c5f47a31de65a/renditions/1217c209d387316ef4acb9f622245c3d",
    welcome:
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/7f83127959ad99547aa65b9351298ac3/revisions/f69eac77825c76be0a13ebb58791092b/renditions/0f6d85675e98c70ee4399f8eaee1b515",
    accommodation:
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/59c4b6414b029a5b18a937069db0db90/revisions/53ccffa6dfd3c8b08905fa0daf852d58/renditions/ed178af2d9becbfa90baef77f770d7a6",
    gifts:
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/5391240c5f1657da7d827f3ed21fad0a/revisions/6e2ff5136489590e8566a6f1dd95e53d/renditions/2d9cdea3debc5057903673367b8f5764",
    galleryOne:
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/b891ab4865629d7a707c2060794c313f/revisions/eb9f65bd9d97fd0f0b38b307da5505c0/renditions/c89f9ad5dd1854babce352d6d30890b3",
    galleryTwo:
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/31605a9f2039d7afad14ab5942d50350/revisions/8c8dabf1fb711e8774ed8382134490ba/renditions/45028b55355a0ec9aa04537e2bf1d46e",
    galleryThree:
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/28c9b28086d87303b85254e77ea7e9d8/revisions/1d250eb6d39ed5eff68efbc5bd18f35d/renditions/a668cbb208e455c94032074947809f28"
  },
  heroGallery: [
    "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/0375ce1642f119ca4a478df665223d20/revisions/bd8ba3b7b25aae170b6c5f47a31de65a/renditions/1217c209d387316ef4acb9f622245c3d",
    "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/7f83127959ad99547aa65b9351298ac3/revisions/f69eac77825c76be0a13ebb58791092b/renditions/0f6d85675e98c70ee4399f8eaee1b515",
    "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/59c4b6414b029a5b18a937069db0db90/revisions/53ccffa6dfd3c8b08905fa0daf852d58/renditions/ed178af2d9becbfa90baef77f770d7a6",
    "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/5391240c5f1657da7d827f3ed21fad0a/revisions/6e2ff5136489590e8566a6f1dd95e53d/renditions/2d9cdea3debc5057903673367b8f5764",
    "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/b891ab4865629d7a707c2060794c313f/revisions/eb9f65bd9d97fd0f0b38b307da5505c0/renditions/c89f9ad5dd1854babce352d6d30890b3",
    "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/31605a9f2039d7afad14ab5942d50350/revisions/8c8dabf1fb711e8774ed8382134490ba/renditions/45028b55355a0ec9aa04537e2bf1d46e",
    "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/28c9b28086d87303b85254e77ea7e9d8/revisions/1d250eb6d39ed5eff68efbc5bd18f35d/renditions/a668cbb208e455c94032074947809f28",
    "https://images.unsplash.com/photo-1464366400600-7168b8af9bc3?auto=format&fit=crop&w=1400&q=86",
    "https://images.unsplash.com/photo-1519225421980-715cb0215aed?auto=format&fit=crop&w=1400&q=86",
    "https://images.unsplash.com/photo-1519167758481-83f550bb49b3?auto=format&fit=crop&w=1400&q=86"
  ],
  galleryDecks: [
    [
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/b891ab4865629d7a707c2060794c313f/revisions/eb9f65bd9d97fd0f0b38b307da5505c0/renditions/c89f9ad5dd1854babce352d6d30890b3",
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/31605a9f2039d7afad14ab5942d50350/revisions/8c8dabf1fb711e8774ed8382134490ba/renditions/45028b55355a0ec9aa04537e2bf1d46e",
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/28c9b28086d87303b85254e77ea7e9d8/revisions/1d250eb6d39ed5eff68efbc5bd18f35d/renditions/a668cbb208e455c94032074947809f28"
    ],
    [
      "https://images.unsplash.com/photo-1519167758481-83f550bb49b3?auto=format&fit=crop&w=1200&q=86",
      "https://images.unsplash.com/photo-1464366400600-7168b8af9bc3?auto=format&fit=crop&w=1200&q=86",
      "https://images.unsplash.com/photo-1519741497674-611481863552?auto=format&fit=crop&w=1200&q=86"
    ],
    [
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/59c4b6414b029a5b18a937069db0db90/revisions/53ccffa6dfd3c8b08905fa0daf852d58/renditions/ed178af2d9becbfa90baef77f770d7a6",
      "https://images.unsplash.com/photo-1506377247377-2a5b3b417ebb?auto=format&fit=crop&w=1200&q=86",
      "https://images.unsplash.com/photo-1520854221256-17451cc331bf?auto=format&fit=crop&w=1200&q=86"
    ],
    [
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/0375ce1642f119ca4a478df665223d20/revisions/bd8ba3b7b25aae170b6c5f47a31de65a/renditions/1217c209d387316ef4acb9f622245c3d",
      "https://lightroom.adobe.com/v2c/spaces/b69aed502dc74169b658a828368f7739/assets/7f83127959ad99547aa65b9351298ac3/revisions/f69eac77825c76be0a13ebb58791092b/renditions/0f6d85675e98c70ee4399f8eaee1b515",
      "https://images.unsplash.com/photo-1519225421980-715cb0215aed?auto=format&fit=crop&w=1200&q=86"
    ]
  ],
  imageFallbacks: {
    hero:
      "https://images.unsplash.com/photo-1523438885200-e635ba2c371e?auto=format&fit=crop&w=2200&q=85",
    welcome:
      "https://images.unsplash.com/photo-1523438885200-e635ba2c371e?auto=format&fit=crop&w=1400&q=85",
    accommodation:
      "https://images.unsplash.com/photo-1519741497674-611481863552?auto=format&fit=crop&w=1400&q=85",
    gifts:
      "https://images.unsplash.com/photo-1520854221256-17451cc331bf?auto=format&fit=crop&w=1400&q=85",
    galleryOne:
      "https://images.unsplash.com/photo-1606216794074-735e91aa2c92?auto=format&fit=crop&w=1200&q=85",
    galleryTwo:
      "https://images.unsplash.com/photo-1511285560929-80b456fea0bc?auto=format&fit=crop&w=1200&q=85",
    galleryThree:
      "https://images.unsplash.com/photo-1529634806980-85c3dd6d34ac?auto=format&fit=crop&w=1200&q=85"
  },
  schedule: [
    { time: "15:00", title: { fr: "Cérémonie", en: "Ceremony" }, text: { fr: "Échange des vœux et bénédiction.", en: "Vows exchange and blessing." } },
    { time: "16:15", title: { fr: "Cocktail", en: "Cocktail" }, text: { fr: "Rafraîchissements, musique douce et photos.", en: "Refreshments, soft music and photos." } },
    { time: "18:00", title: { fr: "Réception", en: "Reception" }, text: { fr: "Accueil des invités dans la salle principale.", en: "Guest welcome in the main reception space." } },
    { time: "19:00", title: { fr: "Dîner", en: "Dinner" }, text: { fr: "Dîner élégant servi à table.", en: "Elegant seated dinner service." } },
    { time: "21:00", title: { fr: "Ouverture de bal", en: "First Dance" }, text: { fr: "Premier moment de danse des mariés.", en: "The couple's first dance." } },
    { time: "21:30", title: { fr: "Soirée", en: "Celebration" }, text: { fr: "Musique, danse et célébration.", en: "Music, dancing and celebration." } }
  ],
  content: {
    fr: {
      nav: { welcome: "Bienvenue", schedule: "Programme", venue: "Lieu", rsvp: "RSVP" },
      hero: {
        script: "Nous serions honorés de célébrer ce jour avec vous.",
        date: "26 décembre 2026",
        location: "Johannesburg, Afrique du Sud",
        rsvpButton: "Confirmer ma présence",
        detailsButton: "Découvrir les détails"
      },
      intro: {
        text:
          "Avec joie et gratitude, nous vous invitons à partager la célébration de notre union. Votre présence rendra cette journée encore plus précieuse."
      },
      countdown: {
        eyebrow: "Avant l'entrée dans la salle de bal",
        title: "Le compte à rebours royal",
        days: "Jours",
        hours: "Heures",
        minutes: "Minutes",
        seconds: "Secondes"
      },
      welcome: {
        eyebrow: "Bienvenue à notre célébration",
        title: "Une nuit imaginée comme une réception royale",
        subtitle:
          "Merci d'avoir confirmé votre présence. Nous sommes profondément honorés de vous compter parmi les personnes qui partageront cette journée si importante pour nous.",
        blocks: [
          { title: "Notre gratitude", text: "Votre présence rendra cette célébration encore plus spéciale, plus chaleureuse et plus mémorable." },
          { title: "L'ambiance", text: "Nous avons imaginé une nuit royale en cristal, entre ivoire, champagne, or doux, chandeliers et élégance intemporelle." },
          { title: "L'invitation", text: "Nous avons hâte de vous accueillir dans une atmosphère raffinée, pensée comme une grande réception de prestige." }
        ]
      },
      schedule: {
        eyebrow: "Jour du mariage",
        title: "Programme du jour",
        subtitle: "Les horaires peuvent être ajustés facilement dans le fichier de configuration."
      },
      venue: {
        eyebrow: "Lieu et carte",
        text:
          "Notre célébration aura lieu à Avianto, un lieu élégant et romantique près de Johannesburg, choisi pour son charme intemporel.",
        location: "Johannesburg, Afrique du Sud",
        date: "Samedi 26 décembre 2026",
        button: "Voir sur Google Maps"
      },
      accommodation: {
        eyebrow: "Hébergement / Accommodation",
        title: "Séjourner à proximité",
        text:
          "Des chambres peuvent être disponibles à Avianto. Cette section accueillera les informations officielles concernant les chambres réservées, les tarifs et les modalités de réservation lorsque les détails seront finalisés.",
        button: "Réserver une chambre"
      },
      gifts: {
        eyebrow: "Cadeaux",
        title: "Votre présence est notre plus beau cadeau",
        text:
          "Si vous souhaitez nous honorer davantage, une contribution pourra être offerte le jour du mariage. Aucun paiement en ligne n'est prévu."
      },
      photos: { eyebrow: "Galerie vivante", title: "Fragments d'une nuit royale" },
      rsvp: {
        eyebrow: "Dernière confirmation",
        title: "RSVP final",
        text:
          "Nous savons que les circonstances peuvent évoluer. Merci de nous confirmer une dernière fois votre présence afin de nous aider à finaliser l'organisation de cette journée exceptionnelle.",
        note: "Votre réponse nous aidera à ajuster les derniers détails avec soin et élégance."
      },
      form: {
        name: "Nom complet",
        email: "Email",
        phone: "Numéro de téléphone",
        attendance: "Présence",
        choose: "Choisir une option",
        yes: "Oui, je serai présent(e)",
        no: "Non, je ne pourrai pas venir",
        guests: "Nombre de personnes",
        language: "Choix de langue",
        message: "Message optionnel",
        submit: "Envoyer ma réponse",
        success: "Merci, votre réponse a été envoyée.",
        error: "Veuillez vérifier les champs obligatoires.",
        sending: "Envoi en cours...",
        emailError: "L'envoi n'a pas abouti. Veuillez réessayer ou nous contacter directement."
      },
      footer: { text: "Johannesburg, Afrique du Sud | 26 décembre 2026" }
    },
    en: {
      nav: { welcome: "Welcome", schedule: "Schedule", venue: "Venue", rsvp: "RSVP" },
      hero: {
        script: "We would be honoured to celebrate this day with you.",
        date: "December 26, 2026",
        location: "Johannesburg, South Africa",
        rsvpButton: "Send RSVP",
        detailsButton: "View details"
      },
      intro: {
        text:
          "With joy and gratitude, we invite you to share in the celebration of our union. Your presence will make this day even more meaningful."
      },
      countdown: {
        eyebrow: "Before entering the ballroom",
        title: "The royal countdown",
        days: "Days",
        hours: "Hours",
        minutes: "Minutes",
        seconds: "Seconds"
      },
      welcome: {
        eyebrow: "Welcome to our celebration",
        title: "A night imagined as a royal reception",
        subtitle:
          "Thank you for confirming your presence. We are deeply honoured to count you among the people who will share this very important day with us.",
        blocks: [
          { title: "Our gratitude", text: "Your presence will make this celebration even more special, warmer and more memorable." },
          { title: "The atmosphere", text: "We have imagined a royal crystal night, between ivory, champagne, soft gold, chandeliers and timeless elegance." },
          { title: "The invitation", text: "We cannot wait to welcome you into a refined atmosphere inspired by a grand reception of prestige." }
        ]
      },
      schedule: {
        eyebrow: "Wedding day",
        title: "Schedule of the day",
        subtitle: "Times can be adjusted easily in the configuration file."
      },
      venue: {
        eyebrow: "Venue and map",
        text:
          "Our celebration will take place at Avianto, an elegant and romantic venue near Johannesburg, chosen for its timeless charm.",
        location: "Johannesburg, South Africa",
        date: "Saturday, December 26, 2026",
        button: "Open in Google Maps"
      },
      accommodation: {
        eyebrow: "Accommodation",
        title: "Stay nearby",
        text:
          "Rooms may be available at Avianto. This section will hold the official details for reserved rooms, rates and booking instructions once everything is finalised.",
        button: "Book a Room"
      },
      gifts: {
        eyebrow: "Gifts",
        title: "Your presence is our greatest gift",
        text:
          "Should you wish to honour us further, a contribution may be offered on the wedding day. No online payment or gift registry is planned."
      },
      photos: { eyebrow: "Living gallery", title: "Fragments of a royal night" },
      rsvp: {
        eyebrow: "Final confirmation",
        title: "Final RSVP",
        text:
          "We know that circumstances can change. Please confirm your presence one final time to help us finalize the organization of this exceptional day.",
        note: "Your response will help us adjust the final details with care and elegance."
      },
      form: {
        name: "Full name",
        email: "Email",
        phone: "Phone number",
        attendance: "Attendance",
        choose: "Choose an option",
        yes: "Yes, I will attend",
        no: "No, I cannot attend",
        guests: "Number of guests",
        language: "Language choice",
        message: "Optional message",
        submit: "Send RSVP",
        success: "Thank you, your RSVP has been sent.",
        error: "Please check the required fields.",
        sending: "Sending...",
        emailError: "The submission could not be sent. Please try again or contact us directly."
      },
      footer: { text: "Johannesburg, South Africa | December 26, 2026" }
    }
  }
};
